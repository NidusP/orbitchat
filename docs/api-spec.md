...
POST   /api/v1/auth/resend-verification   # 需登录；1 次/分钟/用户
...
**邮箱验证（P2-C，默认关闭）**：见 [ADR 25](./decisions/25-email-verification.md)。`EMAIL_VERIFICATION_ENABLED=false` 时注册即写入 `emailVerifiedAt`；开启后注册发验证邮件，MVP 不阻断登录。
...
- 成功 `data`：`{ success: true, emailVerifiedAt }`
- 失败：`VALIDATION_ERROR` 400（无效或过期 token）；`SERVICE_UNAVAILABLE` 503（功能关闭）

**`POST /api/v1/auth/resend-verification`**（需 Bearer）

- 成功 `data`：`{ success: true }`
- 失败：`RATE_LIMITED` 429（1 次/分钟）；`VALIDATION_ERROR` 400（已验证）
...
GET    /api/v1/notifications             # 站内互动/消息通知；cursor 分页
...
`InteractionNotification` 类型（`shared-types`）：

```typescript
type InteractionNotificationType = 'post_liked' | 'post_commented' | 'message_received';

interface InteractionNotification {
  id: string;
  type: InteractionNotificationType;
  actor: PostAuthorSummary;
  post?: { id: string; contentPreview: string };       // post_liked / post_commented
  comment?: { id: string; contentPreview: string };    // post_commented
  conversationId?: string;                           // message_received
  message?: { id: string; contentPreview: string };    // message_received
  readAt: string | null;
  createdAt: string;
}
```

- **`GET /api/v1/notifications`**：Query `cursor`、`limit`（默认 20，最大 50）。返回 `CursorPage<InteractionNotification>`。
- **`GET /api/v1/notifications/unread-count`**：返回 `{ count: number }`。
- **`PATCH /api/v1/notifications/read`**：Body 可选 `{ notificationIds?: string[] }`；省略时标记全部未读为已读。返回 `{ updated: number }`。

**写入规则**（服务端自动，无公开 POST）：
- 他人点赞你的帖子 → `post_liked`
- 他人评论你的帖子 → `post_commented`
- 1:1 私聊收到新消息 → `message_received`（**群聊消息通知 MVP 延后**）
- 自操作（自己点赞/评论/给自己发消息）不生成通知

**客户端导航**：`post_*` → `/posts/{post.id}`；`message_received` → `/messages/{conversationId}`
...
```
POST   /api/v1/uploads                   # multipart: file + purpose (avatar|post|message)
...
- 字段：`file`（二进制）；`purpose`：`avatar` | `post` | `message` | `group_avatar`
- 限制：`avatar` / `group_avatar` ≤ 5MB；`post` / `message` ≤ 10MB；MIME `image/jpeg` | `image/png` | `image/webp`
...
**`PATCH /api/v1/conversations/:id`** 扩展：`avatarUploadId?: string`（仅 `type=group`；owner/admin；提交后 `avatarUrl` 设为 `/api/v1/media/{id}`；与群名/公告独立，不需 `expectedVersion`）
...
interface Conversation {
  ...
  type: 'direct' | 'group';
  title: string | null;
  avatarUrl: string | null; // 群头像；direct 恒为 null
...
发送文字或图片消息。服务端写入 DB 成功后广播 `message.new`。

Request：

```typescript
interface CreateMessageRequest {
  content?: string; // 0..2000 chars after trim；纯图消息可省略或空串
  uploadId?: string; // 可选；须为本人 pending + purpose=message；MVP 每消息最多 1 张
}
```

- `content` 与 `uploadId` 至少其一非空
- 响应 `Message` 含可选 `media: PostMediaItem[]`（与发帖媒体结构相同）
...
修改群 **元数据**（群名 / 公告 / 头像）。仅 `type=group`；需要 **owner** 或 **admin**。`title`、`announcement`、`avatarUploadId` 至少提供一个；群名与公告共享同一 `version`（协作编辑乐观锁），头像更新不使用 `expectedVersion`。
...
interface UpdateGroupConversationRequest {
  ...
  avatarUploadId?: string;     // 本人 pending + purpose=group_avatar
  expectedVersion?: number;     // 更新 title/announcement 时必填；来自 Conversation.version
}
```
