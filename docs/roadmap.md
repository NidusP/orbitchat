...
> **实施计划**：[phase-3-plan.md](./phase-3-plan.md) — Wave 0 并行 ADR/spec；3A 三轨道
...
- [ ] WebSocket 实现（Hono/Bun 原生 vs 库）→ 见 [phase-3-kickoff.md](./phase-3-kickoff.md) ADR 13
- [ ] 消息投递模型（REST 写 + WS 广播）→ ADR 14
- [ ] 多实例广播（Redis Pub/Sub）— **Phase 3A 不做**，3B+ 再评
