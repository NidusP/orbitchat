...
**Phase 1–2、Phase 3A、Phase 4A、Phase 4B（`send_dm`）已交付**；完整 Phase 3 / Phase 4 仍各只做了一部分。

| 已交付 | 说明 |
|--------|------|
| Phase 1 | 用户认证、Profile、Session |
| Phase 2 | Feed、发帖、关注、搜索、评论 |
| **Phase 3A** | 1:1 私聊、REST + `WS /ws/v1/chat`、Web `/messages` |
| **Phase 4A** | AI Chat（小轨）、本地模型 SSE、只读 `search_contact` |
| **Phase 4B（切片）** | `send_dm` 用户确认后发私聊 |

| 下一步（可并行） | 文档 |
|------------------|------|
| 3B 群聊、typing、Redis 多实例 | [phase-3-4-next-plan.md](./phase-3-4-next-plan.md) Track M |
| 4B 更多 Tool、结构化 schema、4A 增强 | 同上 Track A |
| 3.1 推送、4C 音视频/多媒体 | 同上 Track C（延后） |

**交付回顾入口**：[phase-3-agent-closeout.md](./phase-3-agent-closeout.md)
...
聊天 (Messaging)               # Phase 3A ✅ direct；3B 群聊待做
├── 对话 (Conversation)      # 1:1 ✅
├── 群组 (ChatGroup)         # 3B
└── 消息 (Message)           # ✅
