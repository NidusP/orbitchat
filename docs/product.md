...
Orbitchat 的**终极目标是以独立开发者身份交付一款可上线、可演进的社交产品**（用户系统、Feed、IM、AI 助手、通知与多媒体等完整链路）。

在交付过程中会系统积累：**Agent / LLM 应用工程**、**IM 与会话设计**、**对象存储与 Feed 架构**、多端契约与文档驱动开发等能力——这些是手段与副产品，不是项目存在的理由。

**技术底座**：Next.js + Hono 的 pnpm workspace 全栈 Monorepo（Bun 运行时）。

**当前迭代计划**：[phase-indie-product-plan.md](./phase-indie-product-plan.md)（P1 上传/聊天发图/通知测试；P2 群头像/消息通知/邮箱验证）。
...
| **通知系统** | 关注、评论、点赞、聊天、通话通知 | P1 | Phase 2–3 | 站内互动 ✅（点赞/评论）；新消息/推送 ⏸ P2/P3 |
| **AI Chat** | 与 AI 助手对话（个人助手 / 社交场景） | P2 | Phase 4 | ✅ **4A+4B**（FC 写工具 + 井字棋） |
| **个人橱窗** | 用户商品展示与交易（类轻量商城） | P2 | Phase 4–5 |
| **多媒体** | 图片、视频上传和处理 | P2 | Phase 4 | 发帖/头像 ✅ MVP；聊天发图 P1；视频/WebRTC 待做 |
...
**核心功能已合入 `master`（PR #7）**：产品 Shell、i18n/暗色/onboarding、站内通知、发帖配图与头像上传、小轨 Wave 1–5。

| 已交付 | 说明 |
|--------|------|
| Phase 1–2 | 用户、社交 Feed |
| Phase 3A/3B | 私聊、群聊、群管理、邀请、消息策略 |
| Phase 4A/4B | AI 小轨、FC 工具、井字棋 |
| **产品化** | App Shell、暖色 UI、通知中心、上传 MVP |

| 进行中（P1/P2） | 文档 |
|----------------|------|
| 上传清理、聊天发图、通知单测 | [phase-indie-product-plan.md](./phase-indie-product-plan.md) |
| 群头像、新消息通知、邮箱验证 | 同上 |
| 4C 音视频 / Web Push | roadmap（P3） |

**交付回顾**：[archive/phase-3-4-delivery.md](./archive/phase-3-4-delivery.md)
...
**合并交付说明**：[archive/phase-3-4-delivery.md](./archive/phase-3-4-delivery.md)  
